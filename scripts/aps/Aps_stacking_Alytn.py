...
file_path="data/Altyn_Tagh_Fault/aps_data/CorrectedIfgs-model6-defoFlag1-wetFlag1-hydroFlag2-iteraNum1-solver2-ueFixMethod0-weightScheme1-shortBt60-shortBtRelax500"
...
# print(displacement_r3_inc.shape)
...
#calculate the long timebaseline stacking
# 75ifgs, divide into 3 groups, index:0-24,25-50,51-75
part1 = displacement_r3_inc[0:25].sum(axis=0)
part2 = displacement_r3_inc[25:51].sum(axis=0)
part3 = displacement_r3_inc[51:75].sum(axis=0)
long_timebaseline_ifgs=np.array([part1,part2,part3])
baseline1=sum(time_baselines[0:25])
baseline2=sum(time_baselines[25:51])
baseline3=sum(time_baselines[51:75])
long_timebaselines=np.array([baseline1,baseline2,baseline3])

ph_sum_long=np.zeros((207,235))
t_sum_long=0
for i in range(len(long_timebaselines)):
    time_baseline=long_timebaselines[i]
    time_baseline=time_baseline/365.25
    t_sum_long+=time_baseline**2
    ph_sum_long+=time_baseline*long_timebaseline_ifgs[i]
ph_rate_long=ph_sum_long/t_sum_long
deformation_velocity_long=ph_rate_long*(-0.056/(4*np.pi))*1000

plt.imshow(deformation_velocity_long,cmap='jet',aspect='auto',vmin=-5,vmax=5)
plt.savefig(os.path.join(save_path,'velocity_longbaselines.png'))

#save data
with open(os.path.join(save_path,'velocity.pkl'),'wb') as f:
    pickle.dump(deformation_velocity,f)
with open(os.path.join(save_path,'velocity_longbaselines.pkl'),'wb') as f:
    pickle.dump(deformation_velocity_long,f)
