...
ICASAR_settings = {"n_comp": 6,  # number of components to recover with ICA (ie the number of PCA sources to keep)
                   ...
                   "hdbscan_param": (100, 20), # (min_cluster_size, min_samples) Discussed in more detail in Mcinnes et al. (2017). min_cluster_size sets the smallest collection of points that can be considered a cluster. min_samples sets how conservative the clustering is. With larger values, more points will be considered noise.
                   "out_folder": Path('example_spatial_03_n6_APS_cum'),  # outputs will be saved here
...
fig_title_01='deformation_ifgs_ICA_series'
fig_title_02='deformation_ifgs_PCA_series'
fig_title_03='deformation_ifgs_ICA_stacking'
fig_title_04='deformation_ifgs_PCA_stacking'
recover_signal_timeseries_plot(S_ica,A_ica,0,mask,file_path,fig_title_01,phUnw_mean)
deformation_velocity_ica=stacking_insar(S_ica,A_ica,0,mask,file_path,time_baselines_inc,phUnw_mean,fig_title=fig_title_03)
recover_signal_timeseries_plot(S_pca,A_pca,0,mask,file_path,fig_title_02,phUnw_mean)
deformation_velocity_pca=stacking_insar(S_pca,A_pca,0,mask,file_path,time_baselines_inc,phUnw_mean,fig_title=fig_title_04)
...
#save the mask
with open(os.path.join(file_path,'mask.pkl'),'wb') as f:
    pickle.dump(mask,f)
