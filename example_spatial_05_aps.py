...
# Date: 2024/01/19
# Function:
#   ICASAR for Altyn Tagh fault Aps phase corrected
#-------------------------------------------------------------------
#读取数据
import pickle
import numpy as np

filepath_displacement='data/Altyn_Tagh_Fault/temp_correct/displacement_r2.pkl'
filepath_tbaselines='data/Altyn_Tagh_Fault/temp_correct/tbaseline_info.pkl'
...
time_baselines_inc=tbaseline_info['baselines']
time_baselines_inc=np.array(time_baselines_inc)
...
ICASAR_settings = {"n_comp": 5,  # number of components to recover with ICA (ie the number of PCA sources to keep)
                   ...
                   "hdbscan_param": (25, 5), # (min_cluster_size, min_samples) Discussed in more detail in Mcinnes et al. (2017). min_cluster_size sets the smallest collection of points that can be considered a cluster. min_samples sets how conservative the clustering is. With larger values, more points will be considered noise.
                   "out_folder": Path('example_spatial_05_APS_cum'),  # outputs will be saved here
                   ...
                   "ifgs_format": 'cum', # small signals are hard for ICA to extact from time series, so make it easier by creating all possible long temporal baseline ifgs from the incremental data.
...
# print('重建干涉图与原始干涉图之间的RMSE:',x_train_residual_ts)
                                              ...
                                              #%%% 结果判读部分
from icasar.aux1 import visualise_ICASAR_inversion,col_to_ma
...
recover_signal_timeseries_plot(S_ica,A_ica,1,mask,file_path,fig_title_01,phUnw_mean)
#recover_signal_timeseries_plot(S_ica,A_ica,4,mask,file_path,fig_title_02,phUnw_mean)

#使用stacking_insar计算并绘制地表形变平均速率图
deformation_velocity_ica=stacking_insar(S_ica,A_ica,1,mask,file_path,time_baselines_inc,phUnw_mean,fig_title=fig_title_01)
deformation_velocity_pca=stacking_insar(S_pca,A_pca,0,mask,file_path,time_baselines_inc,phUnw_mean,fig_title=fig_title_03)
