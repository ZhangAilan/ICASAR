...
# Date: 2024/05/06
# Function:
#   ICASAR应用于Synthetic数据，数据没有经过APS校正，模式为cumulative
#-------------------------------------------------------------------
import pickle
import numpy as np
...
ICASAR_settings = {"n_comp": 5,  # number of components to recover with ICA (ie the number of PCA sources to keep)
                   ...
                   "out_folder": Path('example_spatial_03_cum'),  # outputs will be saved here
...
deformation_velocity_ica=stacking_insar(S_ica,A_ica,0,mask,file_path,time_baselines_cum,phUnw_mean,fig_title=fig_title_03)
recover_signal_timeseries_plot(S_pca,A_pca,0,mask,file_path,fig_title_02,phUnw_mean)
deformation_velocity_pca=stacking_insar(S_pca,A_pca,0,mask,file_path,time_baselines_cum,phUnw_mean,fig_title=fig_title_04)
